...
export function TenantPaymentChart({ metrics }: Props) {
    const cashlessOrders = metrics.cashless_orders ?? 0;
    const cashOrders = metrics.cash_orders ?? 0;
    const cashlessRevenue = metrics.cashless_revenue ?? 0;
    const cashRevenue = metrics.cash_revenue ?? 0;
    const cashlessPercentage = metrics.cashless_percentage ?? (cashlessOrders + cashOrders > 0 ? Math.round((cashlessOrders / (cashlessOrders + cashOrders)) * 100) : 0);
    const totalOrders = cashlessOrders + cashOrders;
    const totalRevenue = cashlessRevenue + cashRevenue;

    const chartData = useMemo(() => {
        return [
            {
                name: 'cashless',
                value: cashlessOrders,
    ...
    }, [cashlessOrders, cashOrders, cashlessRevenue, cashRevenue]);
        ...
        <Card className="col-span-1 shadow-xs border-border/70 bg-card flex flex-col justify-between">
            <CardHeader className="pb-2">
                ...
                <CardDescription className="text-xs sm:text-sm">
                    Perbandingan transaksi QRIS/Midtrans vs Tunai.
                </CardDescription>
            </CardHeader>
                ...
                <div className="grid grid-cols-2 gap-3 w-full mt-4 pt-3 border-t border-border/50">
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                        ...
                        <div className="truncate">
                            <div className="text-[11px] text-muted-foreground">Cashless ({cashlessOrders})</div>
                            <div className="text-xs font-bold font-mono text-emerald-700 dark:text-emerald-300 truncate">
                                {formatRupiah(cashlessRevenue)}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                        ...
                        <div className="truncate">
                            <div className="text-[11px] text-muted-foreground">Tunai ({cashOrders})</div>
            ...
            <CardFooter className="pt-2 text-xs text-muted-foreground border-t border-border/50 justify-between">
                <span>Total transaksi: {totalOrders} pesanan</span>
